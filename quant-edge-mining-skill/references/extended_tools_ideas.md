...
When prompting an LLM for novel hypotheses, standard inference settings will suppress creative answers. Set your inference configuration during the ideation phase:

* **Temperature (`0.85` - `1.0`):** Increases the likelihood of non-dominant token paths, allowing unconventional connections between indicators.
* **Top-P (`0.92` - `0.95`):** Maintains grammatical coherence while leaving a wide search space for novel vocabulary and logic.
* **Presence Penalty (`0.4`):** Actively penalizes the model for recycling standard words and phrases (e.g., "oversold", "crossover", "golden cross"), forcing it to frame hypotheses with distinct conceptual vocabulary.