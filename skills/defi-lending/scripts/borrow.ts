export {};
/**
 * DeFi Lending — Aave V3 Borrow
 *
 * Borrows an asset from Aave V3.
 *
 * Usage:
 *   export PRIVATE_KEY=0x...
 *   npx tsx borrow.ts <asset_address> <amount> [--chain ethereum|base|...]
 */
...
async function main() {
  ...
  if (!asset || !amount || !privateKey) {
    console.error("Usage: export PRIVATE_KEY=0x... && npx tsx borrow.ts <asset> <amount> [--chain <chain>]");
    process.exit(1);
  }
  ...
  try {
    ...
    console.log("══════════════════════════════════════════════════");
    console.log(`  👻 Aave V3 Borrow on ${chain.name}`);
    console.log("══════════════════════════════════════════════════");
    ...
    const borrowHash = await walletClient.writeContract({
      ...
      abi: POOL_ABI,
      functionName: "borrow",
      args: [asset, amountInUnits, 2n, 0, account.address], // 2 = Variable rate
    });

    console.log("  ✅ Borrow Transaction Sent!");
    console.log(`  Hash:     ${borrowHash}`);
