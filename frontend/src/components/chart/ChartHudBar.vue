<template>
  <div class="h-7 bg-base-200/50 backdrop-blur-xs border-b border-base-content/10 px-3 flex items-center gap-3 text-[11px] font-mono text-base-content/80 overflow-x-auto shrink-0 z-10 select-text">
    ...
    <div v-if="legendData.open !== undefined" class="flex items-center gap-2 shrink-0">
      ...
      <span>O: <strong class="text-base-content">{{ formatPrice(legendData.open, isGoldStrategy) }}</strong></span>
      <span>H: <strong class="text-base-content">{{ formatPrice(legendData.high, isGoldStrategy) }}</strong></span>
      <span>L: <strong class="text-base-content">{{ formatPrice(legendData.low, isGoldStrategy) }}</strong></span>
      <span>C: <strong class="text-base-content">{{ formatPrice(legendData.close, isGoldStrategy) }}</strong></span>
    ...
    <div class="flex items-center gap-2.5 shrink-0">
      ...
      <span v-if="legendData.ema9" class="text-sky-400">EMA9: <strong>{{ formatPrice(legendData.ema9, isGoldStrategy) }}</strong></span>
      <span v-if="legendData.ema21" class="text-indigo-400">EMA21: <strong>{{ formatPrice(legendData.ema21, isGoldStrategy) }}</strong></span>
      <span v-if="legendData.ema200" class="text-amber-400">EMA200: <strong>{{ formatPrice(legendData.ema200, isGoldStrategy) }}</strong></span>
      <span v-if="legendData.hh15" class="text-emerald-400">HH15: <strong>{{ formatPrice(legendData.hh15, isGoldStrategy) }}</strong></span>
      <span v-if="legendData.ll15" class="text-rose-400">LL15: <strong>{{ formatPrice(legendData.ll15, isGoldStrategy) }}</strong></span>
      <span v-if="legendData.bbUpper" class="text-purple-400">BB: <strong>[{{ formatPrice(legendData.bbUpper, isGoldStrategy) }} - {{ formatPrice(legendData.bbLower, isGoldStrategy) }}]</strong></span>
    ...
    <template v-if="inspectedSignal">
      ...
      <div class="flex items-center gap-2.5 shrink-0 bg-base-300/60 px-2 py-0.5 rounded border border-base-content/10">
        ...
        <span class="text-info font-semibold">Entry: ${{ formatPrice(inspectedSignal.entry_price, isGoldStrategy) }}</span>
        <span class="text-error font-semibold">SL: ${{ formatPrice(inspectedSignal.stop_loss, isGoldStrategy) }}</span>
        <span class="text-success font-semibold">TP: ${{ formatPrice(inspectedSignal.take_profit, isGoldStrategy) }}</span>
