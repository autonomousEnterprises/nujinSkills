...
def action_init(target: str, scope: str, context: str, archetype: str = "mean_reversion", initial_rules_input: str = ""):
    ...
    # Load empirical anomaly briefing if generated by anomaly scanner
    briefing_path = os.path.join(AUTORESEARCH_DIR, "empirical_briefing.json")
    if os.path.exists(briefing_path):
        try:
            with open(briefing_path, "r", encoding="utf-8") as f:
                briefing_data = json.load(f)
                state["empirical_macro_regime"] = briefing_data.get("macro_regime_characteristics", {})
                state["empirical_anomalies_count"] = briefing_data.get("statistically_significant_count", 0)
                print(f"{TOOL_NAME} Ingested empirical briefing ({state['empirical_anomalies_count']} statistically significant anomalies detected).")
        except Exception:
            pass
...
def action_scan(data_path: str = "data/candles_15m.csv"):
    from tools.anomaly_scanner import generate_empirical_briefing, print_ascii_briefing
    print(f"{TOOL_NAME} Scanning {data_path} for empirical statistical anomalies and alpha half-life...")
    out_path = os.path.join(AUTORESEARCH_DIR, "empirical_briefing.json")
    briefing = generate_empirical_briefing(data_path, output_path=out_path)
...
def _mutate_numeric_thresholds(expr: str, factor: float) -> str:
    """
    Dynamically scales comparison thresholds (> X, < Y, >= Z, <= W) by factor
    without caring what the indicator or feature name is.
    Zero hardcoded indicator strings!
    """
    import re
    def replacer(match):
        op = match.group(1)
        val = float(match.group(2))
        # If tightening (> 1.0): for > increase threshold; for < decrease threshold
        if factor >= 1.0:
            new_val = val * factor if ">" in op else val * (2.0 - factor)
        else: # Loosening
            new_val = val * factor if ">" in op else val * (2.0 - factor)
        if "." in match.group(2):
            ...
            formatted = f"{int(round(new_val))}" if new_val >= 1 else f"{new_val:.2f}"
...
def _get_dynamic_candidate_constraint(existing_long: str, features_path: str = "") -> Optional[str]:
    """
    Dynamically finds an empirical anomaly from .nujin/empirical_briefing.json
    or an unused column in features_path. Completely data-driven without hardcoding.
    """
    briefing_file = os.path.join(AUTORESEARCH_DIR, "empirical_briefing.json")
    if os.path.exists(briefing_file):
        try:
            ...
            for a in anomalies:
                cond_id = a.get("anomaly_id", "").lower()
                if not cond_id:
                    continue
                # Neutral scale-invariant mathematical primitives
                mapping = {
                    "run_consecutive_down": "consecutive_down >= 3",
                    "run_consecutive_up": "consecutive_up >= 3",
                    "return_shock_downside": "ret_zscore < -2.0",
                    "return_shock_upside": "ret_zscore > 2.0",
                    "upper_wick_rejection": "upper_wick_ratio > 0.45",
                    "lower_wick_rejection": "lower_wick_ratio > 0.45",
                    "volume_effort_absorption": "relative_vol > 2.0",
                    "volume_effort_expansion": "relative_vol > 2.0",
                    "volatility_compression": "relative_range < 0.5"
                }
                for k, v in mapping.items():
                    if k in cond_id and v not in existing_long:
                        return v
                # Dynamic user feature columns
                if "user_feature_" in cond_id:
                    clean_col = cond_id.replace("user_feature_", "").replace("_spike", "").replace("_dip", "")
                    op = ">" if "_spike" in cond_id else "<"
                    cand = f"{clean_col} {op} 0.0"
                    if cand not in existing_long:
                        return cand
        except Exception:
            pass

    # Dynamic fallback: sample unused column directly from user's features dataset
    if features_path and os.path.exists(features_path):
        try:
            import pandas as pd
            sample_df = pd.read_csv(features_path, nrows=20)
            skip = {"time", "timestamp", "open", "high", "low", "close", "volume", "utc_hour"}
            candidates = [c for c in sample_df.columns if c not in skip and c not in existing_long]
            if candidates:
                chosen = candidates[0]
                return f"{chosen} > 0.0"
        except Exception:
...
def mutate_rules(rules: Dict[str, Any], operator: str, failures: List[str], features_path: str = "") -> Dict[str, Any]:
    """
    Universal, indicator-agnostic rule mutation engine.
    Never injects hardcoded indicators; mutates numbers and structure dynamically.
    """
    mutated = dict(rules)
    arch = mutated.get("archetype", "custom")

    if operator == "tighten_thresholds":
        # Mathematically tighten all numerical thresholds in entry expressions
        if mutated.get("entry_long"):
            mutated["entry_long"] = _mutate_numeric_thresholds(mutated["entry_long"], factor=1.10)
        if mutated.get("entry_short"):
            mutated["entry_short"] = _mutate_numeric_thresholds(mutated["entry_short"], factor=1.10)
        mutated["stoploss_pct"] = round(mutated.get("stoploss_pct", 0.02) * 0.90, 4)

    elif operator == "restructure_exit":
        # Check empirical half-life if available
        half_life = None
        briefing_file = os.path.join(AUTORESEARCH_DIR, "empirical_briefing.json")
        if os.path.exists(briefing_file):
            try:
                with open(briefing_file, "r") as f:
                    b_data = json.load(f)
                sig_anom = [a for a in b_data.get("empirical_anomalies_detected", []) if a.get("is_statistically_significant")]
                if sig_anom:
                    half_life = int(sig_anom[0].get("alpha_half_life_bars", 12))
            except Exception:
                pass

        curr_bars = int(mutated.get("max_bars_held", 12))
        if half_life and half_life != curr_bars:
            new_bars = half_life
        else:
            new_bars = int(curr_bars * 1.5) if curr_bars <= 12 else max(4, int(curr_bars * 0.75))

        mutated["max_bars_held"] = new_bars
        if "bars >=" in mutated.get("exit", ""):
            import re
            mutated["exit"] = re.sub(r"bars\s*>=\s*\d+", f"bars >= {new_bars}", mutated["exit"])
        mutated["takeprofit_pct"] = round(max(mutated.get("stoploss_pct", 0.015) * 1.5, mutated.get("takeprofit_pct", 0.035) * 1.15), 4)

    elif operator == "add_constraint":
        candidate = _get_dynamic_candidate_constraint(mutated.get("entry_long", ""), features_path)
        if candidate and candidate not in mutated.get("entry_long", ""):
            mutated["entry_long"] = f"{mutated['entry_long']} and {candidate}" if mutated.get("entry_long") else candidate
            mutated["entry_short"] = f"{mutated['entry_short']} and {candidate}" if mutated.get("entry_short") else candidate
        else:
            # Scale threshold as fallback
            mutated["entry_long"] = _mutate_numeric_thresholds(mutated.get("entry_long", ""), factor=1.08)
            mutated["entry_short"] = _mutate_numeric_thresholds(mutated.get("entry_short", ""), factor=1.08)

    elif operator == "add_negative_example":
        # Add exhaustion/anti-chase filter if body_ratio or volume is present
        if "body_ratio" in mutated.get("entry_long", ""):
            mutated["entry_long"] = _mutate_numeric_thresholds(mutated["entry_long"], factor=0.95)
        else:
            mutated["entry_long"] = f"{mutated['entry_long']} and body_ratio < 0.70" if mutated.get("entry_long") else "body_ratio < 0.70"
            mutated["entry_short"] = f"{mutated['entry_short']} and body_ratio < 0.70" if mutated.get("entry_short") else "body_ratio < 0.70"

    elif operator == "remove_bloat":
        # Prunes the last 'and' clause dynamically without hardcoding
        for side in ["entry_long", "entry_short"]:
            expr = mutated.get(side, "")
            clauses = [c.strip() for c in expr.split(" and ") if c.strip()]
            if len(clauses) > 1:
                mutated[side] = " and ".join(clauses[:-1])
        ...
        # Data-driven plateau break: query top statistically significant anomaly from empirical briefing
        briefing_file = os.path.join(AUTORESEARCH_DIR, "empirical_briefing.json")
        switched = False
        if os.path.exists(briefing_file):
            try:
                ...
                if sig_anom:
                    top_a = sig_anom[0]
                    cond_name = top_a.get("anomaly_id", "")
                    cand_rule = _get_dynamic_candidate_constraint("", features_path)
                    if cand_rule:
                        mutated["entry_long"] = cand_rule
                        mutated["entry_short"] = cand_rule
                        mutated["max_bars_held"] = top_a.get("alpha_half_life_bars", 12)
                        mutated["exit"] = f"bars >= {mutated['max_bars_held']}"
                        switched = True
                        print(f"{TOOL_NAME} Plateau Break: Re-anchored to empirical anomaly '{cond_name}' (t={top_a.get('peak_t_statistic')})")
            except Exception:
                pass

        if not switched:
            # Invert comparison operators in existing rules
            mutated["entry_long"] = _mutate_numeric_thresholds(mutated.get("entry_long", ""), factor=0.85)
            mutated["entry_short"] = _mutate_numeric_thresholds(mutated.get("entry_short", ""), factor=0.85)
            print(f"{TOOL_NAME} Plateau Break: Broadened numeric threshold search space across active hypothesis.")
